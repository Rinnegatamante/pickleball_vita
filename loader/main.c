/* main.c -- Pickleball Pro .so loader
 *
...
#endif
	SDL_ClearError();
	return SDL_CreateWindow(title, 0, 0, w, h, flags);
}
...
SDL_Texture *IMG_LoadTexture_hook(SDL_Renderer *renderer, const char *fname) {
	SDL_Texture *f;
	char real_fname[256];
#ifdef ENABLE_DEBUG
	...
	if (strncmp(fname, "ux0:", 4)) {
		sprintf(real_fname, "%s/assets/%s", data_path, fname);
		f = IMG_LoadTexture(renderer, real_fname);
	} else {
		f = IMG_LoadTexture(renderer, fname);
	}
	return f;
}

Mix_Chunk *Mix_LoadWAV_hook(const char *fname) {
	Mix_Chunk *f;
	char real_fname[256];
#ifdef ENABLE_DEBUG
...
SDL_Renderer *SDL_CreateRenderer_hook(SDL_Window * window, int index, Uint32 flags) {
	...
	printf("SDL_CreateRenderer(%x, %d, %x) returned %x\n", window, index, flags, ret);
	return ret;
}

void exit_hook(int retcode) {
	printf("SDL error: %s\n", SDL_GetError());
	exit(retcode);
}

TTF_Font *TTF_OpenFont_hook(const char *fname, int ptsize) {
	TTF_Font *f;
	char real_fname[256];
#ifdef ENABLE_DEBUG
	...
	if (strncmp(fname, "ux0:", 4)) {
		sprintf(real_fname, "%s/assets/%s", data_path, fname);
		f = TTF_OpenFont(real_fname, ptsize);
	} else {
		f = TTF_OpenFont(fname, ptsize);
	}
	return f;
}
...
static so_default_dynlib default_dynlib[] = {
	...
	{ "Mix_LoadWAV", (uintptr_t)&Mix_LoadWAV_hook },
	{ "SDL_LogInfo", (uintptr_t)&SDL_LogInfo },
	...
	{ "SDL_HasIntersectionF", (uintptr_t)&SDL_HasIntersectionF },
	{ "Mix_PlayChannel", (uintptr_t)&Mix_PlayChannel },
	{ "SDL_GetRendererOutputSize", (uintptr_t)&SDL_GetRendererOutputSize },
	{ "SDL_RenderDrawRect", (uintptr_t)&SDL_RenderDrawRect },
	{ "SDL_RenderCopyEx", (uintptr_t)&SDL_RenderCopyEx },
	{ "SDL_RenderCopyExF", (uintptr_t)&SDL_RenderCopyExF },
	{ "TTF_OpenFont", (uintptr_t)&TTF_OpenFont_hook },
	{ "SDL_RegisterEvents", (uintptr_t)&SDL_RegisterEvents },
	{ "SDL_WaitEvent", (uintptr_t)&SDL_WaitEvent },
	...
	{ "SDL_IntersectFRectAndLine", (uintptr_t)&SDL_IntersectFRectAndLine },
...
void *ctrl_thread(void *argp) {
	...
	for (;;) {
		...
		sceCtrlPeekBufferPositive(0, &pad, 1);
		if (pad.lx < 100 || pad.lx > 156 || pad.ly < 100 || pad.ly > 156) {
			SDL_Event ev;
			ev.type = SDL_MOUSEMOTION;
			ev.motion.type = SDL_MOUSEMOTION;
			ev.motion.state = 1;
			ev.motion.xrel = ((int)pad.lx - 128) / 16;
			ev.motion.yrel = ((int)pad.ly - 128) / 16;
			SDL_PushEvent(&ev);
		}
		old.lx = pad.lx;
		old.ly = pad.ly;
...
void *real_main(void *argv) {
	...
	char fname[256];
	sprintf(data_path, "ux0:data/pickleball");
	...
	*(uintptr_t *)(fake_env + 0x374) = (uintptr_t)GetStringUTFRegion;
	...
	// Starting thread for mouse emulation via left analog
	pthread_t t;
	pthread_attr_t attr;
	pthread_attr_init(&attr);
	pthread_create(&t, &attr, ctrl_thread, NULL);
	
	printf("Jumping on SDL_main\n");
	char *args[2];
	args[0] = "ux0:data/pickleball";
	SDL_main(1, args);
	printf("SDL error: %s\n", SDL_GetError());
